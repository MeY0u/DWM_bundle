...
void on_dwm_evt(dwm_evt_t *p_evt)
{
	...
	case DWM_EVT_USR_DATA_READY:
		...
		dwm_usr_data_read(data, 5);
		printf("User read data: %s\n", data);
...
void app_thread_entry(uint32_t data){
  ...
  /* Set the PAN ID of all the nodes in the network to 0x0002 */ 
  uint16_t panid = 0x0002;
  ...
  while(1){
    ...
    if (node_type == Anchor){
      ...
      int error;
      error = dwm_usr_data_read(data, &len_data);
      if(error == DWM_OK){
          printf("read buffer\n");
      }
      else {
    ...
    /* Read last measured distances to anchors (tag is currently ranging to) and the associated position */
      ...
      * set by dwt_setrxaftertxdelay() has elapsed. */
      ...
      if (rv == DWM_OK) {
        ...
        int error;
        error =dwm_usr_data_write(data, sizeof(data), true);
        if(error == DWM_OK){
          printf("Send buffer\n");
        }
        else {
...
void dwm_user_start(void)
{
	...
	dwm_serial_spi_compile();
       ...
       // port_set_dw1000_slowrate();			
        ...
        if (dwt_initialise(DWT_LOADUCODE) == DWT_ERROR)
        {
          ...
          printf("ERROR: Initialise failed\n");
          while (1) {};
        }
	...
	APP_ERR_CHECK(rv);
        ...
        DWM_EVT_UWB_SCAN_READY | DWM_EVT_UWBMAC_JOINED_CHANGED, NULL);
